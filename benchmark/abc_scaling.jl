...
for INSTANCE_STR in SCATTERING_PROCESSES
    ...
    flush(stdout)
    parse_dag(joinpath(@__DIR__, "abc_graphs", "$INSTANCE_STR.txt"), INSTANCE)
    SUITE["graph_gen"][INSTANCE_STR] = @benchmarkable parse_dag(joinpath(@__DIR__, "abc_graphs", "$name.txt"), proc) setup = (
        proc = $INSTANCE; name = $INSTANCE_STR; GC.gc()
    )

    g = parse_dag(joinpath(@__DIR__, "abc_graphs", "$INSTANCE_STR.txt"), INSTANCE)
...
BenchmarkTools.save("bench_abc.json", result)
@save "bench_abc.jld2" result graph_props node_dicts comp_times
